...
- Load preview documents from an in-memory Blob URL at their exact output width and display them at scale 1 whenever the viewport can contain them. Shrink only as a narrow-window fit fallback. The UI CSP must keep inline styles enabled for the sandboxed preview frame while scripts remain restricted to `self`; otherwise the shared theme CSS is silently blocked.
- Show the mobile profile inside a fixed phone-shaped preview with a 390 px independently scrollable screen. Device chrome is preview-only and must never appear in copied or exported images. The desktop profile remains an undecorated document canvas.
...
- Keep title-bar tools ordered as compact PC/mobile shortcut summaries, settings gear, then GitHub icon on the right. Configure the two shortcuts in separate compact left/right settings rows, each with current shortcut or Off, an edit action, and a disable action only while enabled. Do not add a settings subtitle, header divider, apply button, or redundant shortcut instructions.
- Show the actual configured shortcut beneath the width inside each profile card using compact macOS key symbols. Keep the original 50 px card height, use smaller supporting text, and show Disabled when the corresponding shortcut is off. Profile cards and settings must stay synchronized in both languages.
- Free friend-to-friend distribution does not require a paid Apple developer account. Check the fixed public GitHub latest Release endpoint on startup and every six hours; manual checks share a pending request. Only higher stable versions show a gear red dot. Software Updates opens the trusted Release page for manual download/replacement; do not claim automatic installation or add an unsigned self-overwrite mechanism. Keep network checks separate from offline document/diagram rendering, with timeout and graceful retry.
...
- `Command+Option+T` always captures the PC profile; `Command+Option+M` always captures mobile. Both use current theme/background/footer/quality without changing the preview profile. Preserve the legacy custom shortcut as PC; legacy disabled shortcuts also leave the newly introduced mobile action disabled. Register replacements transactionally and keep old bindings on conflicts.
- Expose only two fixed, logical output profiles: mobile at 390 px with 14 px body text and 16 px canvas padding, and desktop at 800 px with 14 px body text and 24 px canvas padding. Preview and copied/exported output use these same dimensions directly; do not maintain a doubled high-resolution layout or a fixed 0.5 preview transform. Larger windows add side whitespace instead of enlarging the document, and narrower windows may temporarily shrink the whole preview only to fit. Treat desktop as a wide reading layout instead of a stretched mobile card. Width, font size, and padding are profile-owned and are not user-editable.
...
- Recognize fenced Mermaid, Markmap, Graphviz/DOT, Vega-Lite, ECharts, and G2 (`g2`/`antv-g2`) blocks and render them locally as sanitized static SVG. Keep diagram rendering offline, reject external resources, and parse ECharts/Vega-Lite/G2 as JSON5 configuration only; never execute arbitrary user JavaScript. G2 uses v5 declarative options and its CommonJS entry because its ESM build requires bundler resolution.
- Parse diagram fences through Markdown tokens, including quotes and lists; never interpret fences inside a larger code example as real diagrams. Convert Markmap HTML labels to measured native SVG text before sanitization, and keep `markmap-common` as an explicit production dependency so packaged builds retain it.
...
- Diagram visuals should be restrained and document-oriented: neutral nodes, muted categorical colors, readable axes, thin borders, compact whitespace, and matching dark surfaces. Avoid cartoon icons, candy colors, oversized rounded cards, and gradients. Apply defaults through each renderer while retaining explicit author styles. For visual-only changes, do one focused UI smoke pass rather than repeated full suites.
...
- Keep capture quality as a global persisted setting with two tiers: Standard 1x (the lightweight legacy output) and HD 2x (default). It affects copy, export, and Quick Capture only; preview layout and wrapping stay at the selected profile's logical dimensions. Reduce each column's logical height at HD so no captured bitmap exceeds the 14,000 px physical-height safety limit.
- On Retina displays, render the hidden capture window at the zoom required for the selected capture quality so its physical bitmap is generated at the target resolution. Do not render one oversized texture and resize the complete image afterward; that can exceed Chromium's texture limit, fail on the first attempt, and soften text.
...
- Verify mobile 390 px / 14 px and desktop 800 px / 14 px at scale 1 in a sufficiently wide viewport; preview and copied/exported images must keep identical wrapping and proportions, while legacy or custom profile inputs normalize to mobile.
